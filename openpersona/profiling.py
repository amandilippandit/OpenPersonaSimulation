...
try:  # Optional Persona type
    from .agent import Persona  # type: ignore
except Exception:  # pragma: no cover
    Persona = dict  # type: ignore
...
class Profiler:
    ...
    def profile(
        ...
        Args:
            agents: List of Persona instances.
        ...
        """
        # Store original agent objects (Persona or dicts).
        # The Persona API guarantees a .get() method supporting dot notation for persona attributes.
    ...
    def _analyze_persona_composition(self) -> Dict[str, Any]:
        ...
        def _extract_path(agent: Dict[str, Any], path: List[str]) -> Any:
            # Simplified access: rely on Persona.get (supports dot notation) when available.
            joined = ".".join(path)
    ...
    def _compute_attribute_distribution(
        ...
        Args:
            agents: List of agents (Persona or dict)
    ...
    def _get_nested_attribute(self, agent: dict, attribute: str) -> Any:
        """Get attribute from agent supporting both Persona.get() and dict traversal.

        For Persona objects (non-dict with .get() method), uses their .get() which supports
        dot notation. For plain dicts, performs manual dot-path traversal.
        
        Args:
            agent: Persona instance or dict
            attribute: Attribute path (supports dot notation like "occupation.title")
            
        Returns:
            Attribute value or None if not found
        """
        # Check if it's a Persona (has .get() but is NOT a plain dict)
