...
class PersonaFactory(BaseFactory):
    ...
    def __init__(
        ...
        """
        Initialize a PersonaFactory instance.
        ...
        Args:
            ...
            context (str): The context text used to generate the Persona instances.
    ...
    def generate_person_factories(number_of_factories, generic_context_text):
        """
        Generate a list of PersonaFactory instances using OpenAI's LLM.

        Args:
            number_of_factories (int): The number of PersonaFactory instances to generate.
            generic_context_text (str): The generic context text used to generate the PersonaFactory instances.

        Returns:
            list: A list of PersonaFactory instances.
        """
        ...
        if response is not None:
            ...
            for i in range(number_of_factories):
                ...
                factories.append(PersonaFactory(result[i]))
    ...
    def create_factory_from_demography(
        ...
        """
        Create a PersonaFactory instance from a demography description, which can be wither given as a file path or a dictionary
        (but not both).
        ...
        Args:
            ...
            context (str, optional): Additional context text used to generate the Persona instances. Defaults to None.

        Returns:
            PersonaFactory: A PersonaFactory instance.
        """
    ...
    def _clear_factories(cls):
        ...
        """
        PersonaFactory.all_unique_names = (
            []
    ...
    def generate_person(
        ...
        """
        Generate a Persona instance using OpenAI's LLM.
        ...
        Args:
            ...
            attempts (int): The number of attempts to generate a Persona instance.
            post_processing_func (function): A function to apply to the generated agent after it is created.

        Returns:
            Persona: A Persona instance generated using the LLM.
        """
        ...
        if self.population_size is not None:
            ...
            with concurrent_agent_generataion_lock:
                fresh_agent_name = self._unique_full_name(
                    already_generated_names=PersonaFactory._all_used_and_precomputed_names(),
        ...
        if agent_spec is not None:
            ...
            with concurrent_agent_generataion_lock:
                person = Persona(agent_spec["name"])
    ...
    def generate_people(
        ...
        """
        Generate a list of Persona instances using OpenAI's LLM.

        Args:
            number_of_people (int): The number of Persona instances to generate.
            agent_particularities (str): The particularities of the agent.
            temperature (float): The temperature to use when sampling from the LLM.
            attempts (int): The number of attempts to generate a Persona instance.
            post_processing_func (function): A function to apply to the generated agent after it is created.
        ...
        Returns:
            list: A list of Persona instances generated using the LLM.
        """
    ...
    def _initialize_sampling_plan_transaction(self, n, description, context):
        ...
        all_used_names = PersonaFactory._all_used_and_precomputed_names()
        ...
        for i, sample in enumerate(remaining_characteristics_sample):
            ...
            BaseFactory.randomizer.shuffle(all_used_names)
        ...
        PersonaFactory.all_unique_names = list(
            set(PersonaFactory.all_unique_names + new_names)
        )
    ...
    def _unique_full_names(
        ...
        if n > 0:
            ...
            PersonaFactory.all_unique_names = list(
                set(PersonaFactory.all_unique_names + names)
            )
