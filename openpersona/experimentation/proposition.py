...
from openpersona.experimentation import logger
from openpersona.utils import LLMChat, indent_at_current_level
...
class Proposition:
    ...
    def __init__(
        ...
        """
        Define a proposition as a (textual) claim about a target, which can be a World, a Persona or several of any.
        ...
        Args:
            ...
            target (World, Persona, list): the target or targets of the proposition. If not given, it will have to be specified later.
    ...
    def _target_as_list(self, target):
        if target is None:
            return None
        elif isinstance(target, World) or isinstance(target, Persona):
            return [target]
        elif isinstance(target, list) and all(
            isinstance(t, World) or isinstance(t, Persona) for t in target
        ):
            return target
        else:
            raise ValueError(
                "Target must be a World, a Persona or a list of them."
            )
...
def check_proposition(
    ...
    Args:
        target (World, Persona, list): the target or targets of the proposition
        claim (str): the claim of the proposition
