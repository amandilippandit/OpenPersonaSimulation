...
import openpersona.utils as utils
import openpersona.utils.llm
from openpersona import config_manager
from openpersona.agent import AgentOrWorld, CognitiveActionModel, Self, logger
from openpersona.agent.memory import EpisodicConsolidator, EpisodicMemory, SemanticMemory
from openpersona.control import current_simulation, transactional
from openpersona.utils import LLMChat  # Import LLMChat from the appropriate module
from openpersona.utils import JsonSerializableRegistry, name_or_empty, repeat_on_error
...
#######################################################################################################################
@utils.post_init
class Persona(JsonSerializableRegistry):
    """A simulated person in the OpenPersona universe."""
    ...
    def __init__(
        ...
        Args:
            name (str): The name of the Persona. Either this or spec_path must be specified.
        ...
        assert name is not None, "A Persona must have a name."
        self.name = name
    ...
    def _post_init(self, **kwargs):
        ...
        from openpersona.agent.action_generator import (
            ActionGenerator,
        ...
        if not hasattr(Persona, "_image_description_cache"):
            Persona._image_description_cache: dict[str, str] = {}

        self._prompt_template_path = os.path.join(
            os.path.dirname(__file__), "prompts/persona.v2.mustache"
        )
    ...
    def get(self, key):
        """
        Returns the value of a key in the Persona's persona configuration.
    ...
    def include_persona_definitions(self, additional_definitions: dict):
        """
        Imports a set of definitions into the Persona. They will be merged with the current configuration.
    ...
    def define(self, key, value, merge=False, overwrite_scalars=True):
        """
        Define a value to the Persona's persona configuration. Value can either be a scalar or a dictionary.
    ...
    def define_relationships(self, relationships, replace=True):
        """
        Defines or updates the Persona's relationships.
    ...
    def clear_relationships(self):
        """
        Clears the Persona's relationships.
        """
    ...
    def related_to(self, other_agent, description, symmetric_description=None):
        ...
        Args:
            other_agent (Persona): The other agent.
        ...
        Returns:
            Persona: The agent itself, to facilitate chaining.
        """
    ...
    def act(
        ...
        if n is not None:
            assert n < Persona.MAX_ACTIONS_BEFORE_DONE
        ...
        def _commit_action(action, role, content):
            ...
            if (
                self.enable_basic_action_repetition_prevention
                and (Persona.MAX_ACTION_SIMILARITY is not None)
                and (next_action_similarity > Persona.MAX_ACTION_SIMILARITY)
            ):
                logger.warning(
            ...
            if utils.first_non_none(
                communication_display, Persona.communication_display
            ):
        ...
        def aux_act_once_sequence():
            ...
            def _stimuli_payloads_for_current_turn():
                ...
                max_recall = Persona.MAX_IMAGE_STIMULI_TO_RECALL
            ...
            if len(actions) > 0 and len(actions) > Persona.MAX_ACTIONS_BEFORE_DONE:
                actions = actions[: Persona.MAX_ACTIONS_BEFORE_DONE]
    ...
    def see(
        ...
        Returns:
            Persona: ``self``, to allow method chaining.
        """
        from openpersona.utils.media import normalize_image_refs
    ...
    def _describe_images(
        ...
        """
        from openpersona.utils.media import build_multimodal_content_array, hash_image
        ...
        if cache_key in Persona._image_description_cache:
            logger.debug(f"[{self.name}] Image description cache hit for {cache_key[:12]}…")
            return Persona._image_description_cache[cache_key]
        ...
        # Cache the result
        Persona._image_description_cache[cache_key] = description_text
    ...
    def _update_cognitive_state(
        ...
        """
        Update the Persona's cognitive state.
        """
    ...
    def minibio(self, extended=True, requirements=None):
        """
        Returns a mini-biography of the Persona.
    ...
    def load_specifications_from_folder(
        ...
        for file in os.listdir(folder_path):
            if file.endswith(file_suffix):
                ...
                agent = Persona.load_specification(
                    file_path,
    ...
    def encode_complete_state(self) -> dict:
        """
        Encodes the complete state of the Persona, including the current messages, accessible agents, etc.
    ...
    def decode_complete_state(self, state: dict) -> Self:
        """
        Loads the complete state of the Persona, including the current messages,
        and produces a new Persona instance.
        """
        state = copy.deepcopy(state)

        self._accessible_agents = [
            Persona.get_agent_by_name(name) for name in state["_accessible_agents"]
        ]
    ...
    def create_new_agent_from_current_spec(self, new_name: str) -> Self:
        ...
        """
        new_agent = Persona(name=new_name, spec_path=None)
    ...
    def add_agent(agent):
        ...
        if agent.name in Persona.all_agents:
            ...
            Persona.all_agents[agent.name] = agent
    ...
    def has_agent(agent_name: str):
        ...
        """
        return agent_name in Persona.all_agents
    ...
    def set_simulation_for_free_agents(simulation):
        ...
        if desired.
        """
        for agent in Persona.all_agents.values():
    ...
    def get_agent_by_name(name):
        ...
        """
        if name in Persona.all_agents:
            return Persona.all_agents[name]
        else:
            return None
    ...
    def all_agents_names():
        ...
        """
        return list(Persona.all_agents.keys())
    ...
    def get_global_cost_stats():
        ...
        num_agents = len(Persona.all_agents)
