...
from openpersona.utils import LLMChat
...
class ResultsReporter:
    ...
    def report_from_agents(
        self,
        agents: Union[Persona, World, List[Persona]],
        ...
        Args:
            agents: Single agent, World, or list of agents to interview.
    ...
    def report_from_interactions(
        ...
        Args:
            agents: Single agent, World, or list of agents to analyze.
            report_title: Title for the generated report.
    ...
    def _extract_agents(self, agents) -> List[Persona]:
        """Extract a list of Persona objects from various input types."""
        if isinstance(agents, Persona):
            return [agents]
        elif isinstance(agents, World):
            return agents.agents
        elif isinstance(agents, list):
            return agents
        else:
            raise ValueError(
                "Agents must be a Persona, World, or list of Persona objects."
            )

    def _interview_agent(self, agent: Persona, reporting_task: str) -> str:
        """Interview a single agent about the reporting task."""
        if self.verbose:
            logger.debug(f"Interviewing agent {agent.name} about: {reporting_task}")

        # Following OpenPersona patterns - directly interact with the agent
        prompt = f"""
