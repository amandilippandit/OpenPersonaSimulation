...
class Simulation:
    ...
    def __init__(self, id="default", cached_trace:list=None):
        ...
        self.factories = [] # e.g., PersonaFactory instances
        ...
        self.cache_path = f"./openpersona-{id}.cache.json" # default cache path
    ...
    def begin(self, cache_path:str=None, auto_checkpoint:bool=False):
        ...
        from openpersona.factory.persona_factory import PersonaFactory
    ...
    def _decode_simulation_state(self, state: dict):
        ...
        for environment_state in state["environments"]:
            try:
                ...
                if World.communication_display:
        ...
        for agent_state in state["agents"]:
            try:
                ...
                if agent.environment is None:
                    if Persona.communication_display:
...
class Transaction:
    ...
    def __init__(self, obj_under_transaction, simulation, function, *args, **kwargs):
        ...
        if simulation is not None:
            if hasattr(obj_under_transaction, 'simulation_id') and obj_under_transaction.simulation_id is not None:
                ...
                # if is a Persona, add the agent to the simulation
                if isinstance(obj_under_transaction, Persona):
                    simulation.add_agent(obj_under_transaction)
                    logger.debug(f">>>>>>>>>>>>>>>>>>>>>>> Added agent {obj_under_transaction} to simulation {simulation.id}.")

                # if is a World, add the environment to the simulation
                elif isinstance(obj_under_transaction, World):
                    simulation.add_environment(obj_under_transaction)
                
                # if is a BaseFactory, add the factory to the simulation
                elif isinstance(obj_under_transaction, BaseFactory):
                    ...
                    raise ValueError(f"Object {obj_under_transaction} (type = {type(obj_under_transaction)}) is not a Persona or World instance, and cannot be captured by the simulation.")
        ...
        if self.simulation is None or self.simulation.status == Simulation.STATUS_STOPPED:
            ...
            if self.simulation._is_transaction_event_cached(event_hash, 
                ...
                if not self.simulation.is_under_parallel_transactions():
                    ...
                    # Output encoding/decoding is used to preserve references to Persona and World instances
    ...
    def _encode_function_output(self, output) -> dict:
        ...
        if output is None:
            return None
        elif isinstance(output, Persona):
            return {"type": "PersonaRef", "name": output.name}
        elif isinstance(output, World):
            return {"type": "WorldRef", "name": output.name}
        elif isinstance(output, BaseFactory):
            return {"type": "BaseFactoryRef", "name": output.name}
        elif isinstance(output, list):
            encoded_list = []
            for item in output:
                if isinstance(item, Persona):
                    encoded_list.append({"type": "PersonaRef", "name": item.name})
                elif isinstance(item, World):
                    encoded_list.append({"type": "WorldRef", "name": item.name})
                elif isinstance(item, BaseFactory):
                    encoded_list.append({"type": "BaseFactoryRef", "name": item.name})
                else:
    ...
    def _decode_function_output(self, encoded_output: dict):
        ...
        if encoded_output is None:
            return None
        elif encoded_output["type"] == "PersonaRef":
            return Persona.get_agent_by_name(encoded_output["name"])
        elif encoded_output["type"] == "WorldRef":
            return World.get_environment_by_name(encoded_output["name"])
        elif encoded_output["type"] == "BaseFactoryRef":
            return BaseFactory.get_factory_by_name(encoded_output["name"])
        elif encoded_output["type"] == "List":
            decoded_list = []
            for item in encoded_output["value"]:
                if item["type"] == "PersonaRef":
                    decoded_list.append(Persona.get_agent_by_name(item["name"]))
                elif item["type"] == "WorldRef":
                    decoded_list.append(World.get_environment_by_name(item["name"]))
                elif item["type"] == "BaseFactoryRef":
                    decoded_list.append(BaseFactory.get_factory_by_name(item["name"]))
                else:
